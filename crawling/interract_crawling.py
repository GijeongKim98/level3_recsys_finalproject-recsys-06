...
def main(args, logger) -> None:
    ...
    user_list = pd.read_csv(os.path.join('data', 'user_data.csv'))['handle'][args.start_user_index:args.end_user_index + 1].to_list()
    ...
    for temp_index, temp_user in enumerate(user_list):
        logging.info(f'Index number: {temp_index + args.start_user_index}\tUser name: {temp_user}')
        ...
        while total_page_num is None or total_page_num >= page_num:
            ...
            if response.status_code == 200:
                ...
                correct_output_df = pd.concat([correct_output_df, temp_df], ignore_index=True)
            ...
            # If response code from HTTP request is not any of the above
                ...
                if error_num >= args.errornum:
                    ...
                    correct_output_df.to_csv(os.path.join('data', f'correct_{args.start_user_index}_{args.start_user_index + temp_index}_data.csv'), index=False)
        ...
        while total_page_num is None or total_page_num >= page_num:
            ...
            if response.status_code == 200:
                ...
                # Update dataframe using data
                tried_output_df = pd.concat([tried_output_df, temp_df], ignore_index=True)
            ...
            # If response code from HTTP request is not any of the above
                ...
                if error_num >= args.errornum:
                    # Save current results
                    correct_output_df.to_csv(os.path.join('data', f'correct_{args.start_user_index}_{args.start_user_index + temp_index}_data.csv'), index=False)
                    tried_output_df.to_csv(os.path.join('data', f'tried_{args.start_user_index}_{args.start_user_index + temp_index}_data.csv'), index=False)
        ...
        correct_output_df.to_csv(os.path.join('data', f'temp_correct_data.csv'), index=False)
        tried_output_df.to_csv(os.path.join('data', f'temp_tried_data.csv'), index=False)

    # Get incorrect output dataframe
    correct_output_df['answerCode'] = 1

    # Merage data
    tried_output_df = tried_output_df.merge(correct_output_df, on=['userId', 'problemId'], how='left')
    ...
    tried_output_df.to_csv(os.path.join('data', f'interraction_{args.start_user_index}_{args.end_user_index}_data.csv'), index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', '--logging', type=str, default='INFO', help='Set logging level.')
    parser.add_argument('-s', '--start_user_index', type=int, default=10000, help='The start of the problem page number.')
    parser.add_argument('-e', '--end_user_index', type=int, default=10001, help='The end of the problem page number.')
    parser.add_argument('-m', '--max_error_num', type=int, default=5, help='Number of times the code can get a unknown response code before exiting.')
