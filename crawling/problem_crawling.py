...
def main(args, logger) -> None:
    ...
    while total_page_num is None or page_num <= total_page_num:
        ...
        if response.status_code == 200:
            ...
            output_df = pd.concat([output_df, pd.DataFrame(response.json()['items'])], ignore_index=True)
        ...
        # If response code from HTTP request is not any of the above
            ...
            if error_num >= args.max_error_num:
                # Save results
                output_df.to_csv(os.path.join('data', f'problem_{page_num}_data.csv'), index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-e', '--max_error_num', type=int, default=5, help='Number of times the code can get a unknown response code before exiting.')
    ...
    logging.basicConfig(filename=os.path.join('log', f'{create_time}.txt'),
                        filemode='a',
